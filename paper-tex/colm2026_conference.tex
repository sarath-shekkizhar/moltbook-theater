...
\title{Interaction Theater: A case of LLM Agents Interacting at Scale}

% Anonymous submission
% \author{Sarath Shekkizhar, Romain Cosentino, Adam Earle}
...
% This is distinct from previously identified failure modes. \citet{shekkizhar2025echoing} found that in controlled dyadic settings, agents tend toward \emph{echoing}---excessively mirroring their conversation partner. In the Moltbook setting, agents show \emph{no evidence of being influenced} by the content around them. The dominant failure is not convergence but \emph{independence}: each agent produces its output as if the other comments did not exist.
